...
class ProcessFolderCreation(Hook):
    def execute(self, items, preview_mode, **kwargs):
        """
        Creates a list of files and folders with 777 permissions.

        The default implementation creates files and folders recursively using
        777 permissions (read, write, execute for owner, group, and others).
            ...
            for i in items:
                ...
                if action in ["entity_folder", "folder"]:
                    ...
                    if not os.path.exists(path):
                        if not preview_mode:
                            # create the folder using 777 permissions
                    ...
                    if not os.path.lexists(path):
                        if not preview_mode:
                            ...
                            # set 777 permissions for the symlink
                            os.chmod(path, 0o777)
                        locations.append(path)
